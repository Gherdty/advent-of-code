...
export const getTotalWinnings = () => {
  ...
  for (let i = 0; i < cardPointValues.length; i += 1) {
    ...
    if (repetitions.includes(5)) {
      cardTypes[6].push(cardPointValues[i])
    } else if (repetitions.includes(4)) {
      cardTypes[5].push(cardPointValues[i])
    } else if (repetitions.includes(3)) {
      if (repetitions.includes(2)) {
        cardTypes[4].push(cardPointValues[i])
      } else {
        cardTypes[3].push(cardPointValues[i])
      }
    } else if ((repetitions[0] === 2) && (repetitions[1] === 2)) {
      cardTypes[2].push(cardPointValues[i])
    } else if (repetitions[0] === 2) {
  ...
  const sortedNumbers = [10, 9, 4, 8, 2].sort((a, b) => {
    if (a < b) return - 1
    else return 0
  })
